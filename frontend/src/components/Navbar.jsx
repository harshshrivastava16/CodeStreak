  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-[#18181b] bg-opacity-95 shadow-lg border-b border-gray-800">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 py-3">
        <Link to="/" className="text-2xl font-extrabold bg-gradient-to-r from-lime-400 via-blue-400 to-purple-500 bg-clip-text text-transparent">
          CodeStreak
        </Link>
            ...
            <div className="hidden md:flex items-center gap-6">
              {navLinks.map(link => (
                <Link
                  key={link.to}
                  to={link.to}
                  className={`text-lg font-semibold px-3 py-1 rounded transition duration-150 ${
                    location.pathname === link.to
                      ? "bg-gradient-to-r from-lime-400 via-blue-400 to-purple-500 text-black shadow"
                ...
                <div className="w-64 bg-[#18181b] h-full shadow-2xl p-6 flex flex-col gap-6 animate-slide-in">
                  ...
                  {navLinks.map(link => (
                    <Link
                      key={link.to}
                      to={link.to}
                      className={`text-lg font-semibold px-3 py-2 rounded transition duration-150 ${
                        location.pathname === link.to
                          ? "bg-gradient-to-r from-lime-400 via-blue-400 to-purple-500 text-black shadow"
                          : "text-gray-300 hover:text-lime-400"
                      }`}
